...
|编号|场景|输入|问题|分类|修复方向|
|-|-|-|-|-|-|
|BC-01|info\_check|"吃了饭"|模型直接返回 is\_enough=true|追问类|Prompt 增加模糊表达识别规则|
|BC-02|meal\_analysis|"一碗牛肉面"|热量估算波动大（400-800kcal）|准确性类|Prompt 增加份量参考锚点|
|BC-03|meal\_analysis|用户减脂，吃了炸鸡汉堡|feedback 只说热量数字，不关联目标|语气类|Prompt 要求 feedback 关联用户目标|
|BC-04|daily\_summary|三餐中有一餐是加餐（零食）|AI 未将加餐数据纳入每日合计|准确性类|后端拼接每日数据时包含加餐|
|BC-05|主动触达|漏餐检测触发|文案像系统通知，"请记录晚餐"太生硬|语气类|Prompt 增加语气要求：像朋友轻轻提醒|
...
**证实。** 传统 App 要求搜索食物、调份量、选单位，用户记录意愿随操作繁琐度上升而骤降。食光记用自然语言输入——用户只需说"食堂西红柿炒蛋鸡腿青菜"，AI 自动追问补充，无需查数据库或估克重。单次记录平均耗时 <30 秒。

**假设二：AI 营养估算质量可被用户接受**

**证实。** 用户不输入精确克重时，AI 通过常识推理估算（"一碗牛肉面≈550kcal"）。用户更在意 feedback 的可执行性（"蛋白质够了，加份蔬菜"），而非数字精确到克。±20% 的估算偏差在可接受范围内。

**假设三：多轮追问不会造成用户流失**

**存疑。** 追问 1 次用户基本接受，追问 2 次时部分用户放弃。MVP 阶段缺少完整数据支撑，需上线后通过追问率埋点进一步验证。已设定追问上限 2 次作为兜底。

**假设四：AI 能弥补用户不输入精确克重的短板**

**部分证实。** 对常见中式餐饮（食堂、外卖、家庭餐），AI 基于常识的估算可信。但对罕见食材、小众菜系、跨文化食物，估算偏差明显增大。需上线 RAG 食物营养库后重新验证。
